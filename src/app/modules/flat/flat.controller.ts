...
const createVariant = catchAsync(async (req: Request, res: Response) => {
  ...
  const payload = await flatValidation.createVariantSchema.parseAsync(rawPayload ?? {});
  const files = (Array.isArray(req.files) ? req.files : []) as Express.Multer.File[];
  const result = await flatService.createVariant(userId, propertyId as string, payload, files);
...
const updateVariant = catchAsync(async (req: Request, res: Response) => {
  const userId = req.user?.id as string;
  const { id } = req.params;
  const result = await flatService.updateVariant(userId, id as string, req.body);
